  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        // CTA verde de la app (reemplaza los `bg-accent` puestos a mano).
        accent:
        ...
        // Confirmaciones / estados de éxito (verde sólido).
        success:
          'bg-success text-accent-foreground hover:bg-success/90 focus-visible:ring-success/40',
        // Advertencias (ámbar).
        warning:
          'bg-warning text-accent-foreground hover:bg-warning/90 focus-visible:ring-warning/40',
        ghost:
...
function Button({
  ...
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean
    /** Muestra un spinner y deshabilita el botón. Ignorado con asChild. */
    loading?: boolean
  }) {
  const Comp = asChild ? Slot : 'button'

  // Con asChild (Slot) sólo se permite un hijo: no inyectamos spinner.
  if (asChild) {
    return (
      <Comp
        data-slot="button"
