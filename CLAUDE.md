...
- **Dónde se asigna:** Configuración → Usuarios → (lápiz) → bloque «Acceso».
  Nivel (administrador / editor / visualizador) y, para los que no son
  administradores, los checkboxes de secciones (con marcado por grupo entero y
  botones Todas/Ninguna). Guardar con TODAS marcadas escribe `null`, no la
  lista completa: así el usuario ve también las secciones que se agreguen en
  el futuro.
